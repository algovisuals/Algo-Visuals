...
export function createRandomGraph(
  ...
  for (let i = 0; i < nodeCount; i++) {
    ...
    if (nodeColorOptions) {
      ...
      node.fillColor =
        nodeColorOptions.defaultFillColor || "hsl(210, 100%, 40%)";
      // Explicitly set useGradient to avoid undefined
      node.useGradient =
        nodeColorOptions.useGradient !== undefined
          ? nodeColorOptions.useGradient
          : true;
    }
  ...
  while (unconnected.size > 0) {
    ...
    const toId =
      Array.from(unconnected)[Math.floor(Math.random() * unconnected.size)];
...
export const GraphVisualizer: FC<GraphVisualizerProps> = ({
  ...
  useEffect(() => {
    ...
    currentHighlightedNodes.forEach((hl) =>
      highlightedNodesMap.set(hl.nodeId, {
        ...hl,
    ...
    const bgGradientStart =
      backgroundOptionsRef.current?.gradientStart ||
      (isDarkMode ? "hsl(0, 0.00%, 11.80%)" : "hsl(220, 13%, 95%)");
    const bgGradientEnd =
      backgroundOptionsRef.current?.gradientEnd ||
      (isDarkMode ? "hsl(240, 87.00%, 30.20%)" : "hsl(220, 13%, 90%)");
    const useBackgroundGradient =
      backgroundOptionsRef.current?.useGradient !== false; // Default to true
    const defaultNodeGradientStart = isDarkMode
      ? "hsl(0, 98.00%, 38.60%)"
      : "hsl(210, 100%, 50%)";
    const defaultNodeGradientEnd = isDarkMode
      ? "hsl(210, 100%, 40%)"
      : "hsl(210, 100%, 40%)";
    ...
    svg
      ...
      .attr(
        "fill",
        useBackgroundGradient ? "url(#gradient-bg)" : bgGradientStart,
      )
      .attr("rx", 12)
      .attr("ry", 12);
    ...
    highlightedNodes.forEach((hl) => {
      ...
      const lighterColor =
        d3.color(primaryColor)?.brighter(0.3)?.toString() || primaryColor;
    ...
    const link = linkGroup
      ...
      .attr("stroke-opacity", (d) =>
        highlightedEdgesMap.has(d.id) ? 0.8 : 0.5,
      )
      .attr("stroke-width", (d) => {
    ...
    shadow
      ...
      .attr(
        "flood-color",
        isDarkMode ? "hsl(220, 13%, 5%)" : "hsl(220, 13%, 10%)",
      );
    ...
    const node = nodeGroup
      ...
      .attr("filter", (d) =>
        highlightedNodesMap.has(d.id) ? "url(#glow)" : "url(#shadow)",
      )
      .attr("stroke", (d) => {
        if (highlightedNodesMap.has(d.id)) {
      ...
      .on("mouseenter", function (event, d) {
        ...
        d3.select(this)
          ...
          .attr("stroke-width", () =>
            highlightedNodesMap.has(node.id) ? 3 : 2.5,
          );
      })
      .on("mouseleave", function (event, d) {
        ...
        d3.select(this)
          ...
          .attr("r", nodeRadius)
          .attr("stroke-width", () =>
            highlightedNodesMap.has(node.id) ? 2 : 1.5,
          );
      })
      .call(
        d3
    ...
    const text = svg
      ...
      .attr("font-weight", (d) =>
        highlightedNodesMap.has(d.id) ? "700" : "500",
      )
    ...
    const edgeLabelBackground = svg
      ...
      .attr(
        "fill",
        isDarkMode ? "rgba(30, 41, 59, 0.6)" : "rgba(255, 255, 255, 0.7)",
      )
      .attr("rx", 3)
      .attr("ry", 3)
      .attr("stroke", (d) => {
    ...
    const edgeLabels = svg
      ...
      .attr("font-weight", (d) =>
        highlightedEdgesMap.has(d.id) ? "600" : "400",
      )
      .attr("text-anchor", "middle")
    ...
    const optimalLinkDistance =
      Math.min(width, height) / Math.max(4, Math.sqrt(nodes.length));
    ...
    const simulation = d3
      ...
      .force(
        "link",
        (() => {
          const forceLink = d3.forceLink<Node, Edge>(edges).id((d) => d.id);
          ...
          d3.select(edgeLabelBackground.nodes()[i])
            .attr(
              ...
              midX +
                offsetX -
                parseInt(
                  d3.select(edgeLabelBackground.nodes()[i]).attr("width"),
                ) /
                  2,
            )
            .attr(
              ...
              midY +
                offsetY -
                parseInt(
                  d3.select(edgeLabelBackground.nodes()[i]).attr("height"),
                ) /
                  2,
            );
  ...
  useEffect(() => {
    ...
    svg.selectAll<SVGCircleElement, Node>("circle").each(function (d: Node) {
      ...
      node
        ...
        .attr(
          "stroke",
          isHighlighted ? highlightedNodesMap.get(d.id)!.color : "white",
        )
    ...
    svg
      ...
      .attr("stroke-opacity", (d: Edge) =>
        d.id && highlightedEdgesMap.has(d.id) ? 0.8 : 0.5,
      );

    // Update the edge labels
    svg
      .selectAll<SVGRectElement, Edge>("rect")
      .transition()
      .duration(300)
      .attr("opacity", (d: Edge) => {
