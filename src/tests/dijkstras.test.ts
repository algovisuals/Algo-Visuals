...
describe("Dijkstra Algorithm", () => {
  ...
  test("should correctly record algorithm steps", () => {
    ...
    expect(Array.from(initialStep.unvisited).sort()).toEqual(
      ["A", "B", "C", "D"].sort(),
    );
  ...
  test("should be able to find shortest path using reconstructPath", () => {
    ...
    const pathResult = reconstructPath(
      endNodeId,
      startNodeId,
      result.previous,
      result.distances,
  ...
  test("debug graph structure", () => {
    ...
    for (const nodeId in graph.nodes) {
      ...
      while (edge !== null) {
        debugLog(
          DEBUG,
          `  Edge to ${edge.to_node.id} with weight ${edge.data}`,
        );
        edge = edge.next_from;
        count++;
      }
    ...
    result.steps.forEach((step, index) => {
      ...
      debugLog(
        DEBUG,
        `  Distances: ${[...step.distances.entries()].map(([k, v]) => `${k}:${v}`).join(", ")}`,
      );
